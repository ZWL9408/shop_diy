package com.ssh.shop.order.dao;
...
public class OrderDao extends HibernateDaoSupport {
	...
	public List<Order> findPageByUid(Integer uid, int begin, int limit) {
		...
		List<Order> list = this.getHibernateTemplate().execute(
				(HibernateCallback<List<Order>>) new PageHibernateCallback<Order>(hql, new Object[] { uid },
						begin, limit));
		if (list != null && list.size() > 0) {
			return list;
		}
	...
	public List<Order> findByPage(int begin, int limit) {
		...
		List<Order> list = this.getHibernateTemplate().execute(
				(HibernateCallback<List<Order>>) new PageHibernateCallback<Order>(hql, null, begin, limit));
		return list;
	}
